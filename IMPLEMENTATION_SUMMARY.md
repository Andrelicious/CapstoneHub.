...
## Build & Tooling Notes (March 2026)

### Package Manager Policy
- Repository standard is **pnpm lockfile only** (`pnpm-lock.yaml`)
- `package-lock.json` was removed to avoid mixed lockfile behavior and workspace root inference warnings

### Next.js Build Warning Fixes
- Added `turbopack.root` in `next.config.mjs` to explicitly set the workspace root
- Migrated deprecated middleware convention from `middleware.ts` to `proxy.ts`

### Remaining Non-Blocking Warning
- `baseline-browser-mapping` freshness warning may still appear during build output
- This warning is upstream data-age related and does not block build success
