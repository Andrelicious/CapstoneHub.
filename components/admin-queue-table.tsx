...
export function AdminQueueTable({ submissions }: AdminQueueTableProps) {
  ...
  const filteredSubmissions = submissions.filter((sub) => {
    ...
    const matchesProgram = !selectedProgram || sub.program === selectedProgram
    ...
    <div className="space-y-4">
      ...
      {filteredSubmissions.length === 0 ? (
        <div className="text-center py-12 px-4">
          ...
          <p className="text-muted-foreground">No submissions are currently pending review.</p>
        </div>
      ) : (
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-white/10">
                ...
                <th className="text-left py-3 px-4 text-sm font-semibold text-muted-foreground">OCR Status</th>
              ...
              {filteredSubmissions.map((submission) => (
                <tr key={submission.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                  ...
                  <td className="py-3 px-4">
                    <SubmissionStatusBadge status="pending_admin_review" />
                  </td>
